...
function parse_css_percentage(str) {
    if (str[str.length - 1] !== "%")
        return null;
...
function parse_css_float(str) {
    var _a;
    return (_a = parse_css_percentage(str)) !== null && _a !== void 0 ? _a : clamp_css_float(Number(str));
}
...
function parse_hex_charcode(code) {
    if (code < 48 /* "0" */)
        return NaN;
    else if (code <= 57 /* "9" */)
        return code - 48;
...
function rotation_to_degrees(s) {
    if (s.endsWith("deg")) {
 ...
 * @returns {RGBA | null}
*/
export function parseCSSColor(css_str) {
    ...
    let ep = str.indexOf(")");
    if (ep === -1)
        ep = str.length; // browsers seems to accept such shit
    if (op !== -1 && (ep + 1 === str.length || ep === str.length)) {
        ...
        switch (fname) {
            case "rgba":
            case "rgb":
                result = [...params.map(parse_css_int), alpha];
