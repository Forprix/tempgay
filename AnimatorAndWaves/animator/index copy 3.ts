...
function siz_(e) {
    ...
    if (e.shiftKey) {
        ...
        // (resizeStartSize.y / resizeStartSize.x)
        const v = Vec2(
            daun.x / 2,
            daun.y / 2
        ).rotate(rotation).mul(
        ...
        size.set(
            (resizeStartSize.x + v3.x * resizingDirection.x * 2),
            (resizeStartSize.y + v3.y * resizingDirection.y * 2)
        )
    }
    else {
        const v = Vec2(
...
el3.on('mousedown', e => {
    ...
    rotateRotationMem.set(
        e.clientX - r.left - (pos.x + rotationCenter.x),
        e.clientY - r.top - (pos.y + rotationCenter.y)
    )
})
...
function rot_(e) {
    ...
    let newMem = Vec2(e.clientX - r.left - (pos.x + rotationCenter.x), e.clientY - r.top - (pos.y + rotationCenter.y))
...
// Нельзя передвигать кадры

// Сделано: Пофикшен сервер
// Сделано: Брать серединные значения
// Сделано: Передвижение кадров
// Сделано: Ctrl + V изображения
// Сделано: Повороты позволяют обходить лимиты
