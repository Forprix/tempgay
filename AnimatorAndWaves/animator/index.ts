...
loop(async () => {
    ...
    if (dragging) {
        ...
        if (draw) {
            document.body.style.cursor = `url("data:image/svg+xml,${
                encodeURIComponent(`<svg width="28" height="24" viewBox="-1 -1 27 23" xmlns="http://www.w3.org/2000/svg"><path transform="rotate(${floor(cursorAngle1 * 30) / 30} 13 11)" d="M20.3536 6.64645L20.2071 6.5H20H19H18.5V7V9.5H7.5V7V6.5H7H6H5.79289L5.64645 6.64645L1.64645 10.6464L1.29289 11L1.64645 11.3536L5.64645 15.3536L5.79289 15.5H6H7H7.5V15V12.5H18.5V15V15.5H19H20H20.2071L20.3536 15.3536L24.3536 11.3536L24.7071 11L24.3536 10.6464L20.3536 6.64645Z" fill="white" stroke="black"/></svg>`)
            }") 12 12, auto`
        document.body.classList.remove('no-hard-cursor')
        }
    } 
...
function selectPart(part) {
    ...
    if (selectedPart == null) {
        ...
        paramsEl.style.height = '0'
        paramsEl.style.padding = '0'
        el1.remove()
        el2.remove()
        el4.remove()
        setCursor(1, null)
    ...
    paramsEl.style.removeProperty('height')
    paramsEl.style.removeProperty('padding')
...
function siz_(e) {
    ...
    if (!e.shiftKey) { // Без шифта всё работает идеально!!!
        ...
        if (e.altKey) {
            if (abs(resizingDirection.x) == 1 && abs(resizingDirection.y) == 1) {
                const gayX = (resizeStartSize.x + (rotatedCursorDelta.y * resizingDirection.y * (resizeStartSize.x / resizeStartSize.y))) / resizeStartSize.x
                const gayY = (resizeStartSize.y + (rotatedCursorDelta.x * resizingDirection.x * (resizeStartSize.y / resizeStartSize.x))) / resizeStartSize.y

                const wtf = Vec2(
                    (resizeStartSize.x + rotatedCursorDelta.x),
                    (resizeStartSize.y + rotatedCursorDelta.y)
                )
                if (abs(gayY) > abs(gayX)) {
                ...
                size.set(
                    abs(resizeStartSize.x + rotatedCursorDelta.x) * sign(wtf.x) * (resizingDirection.x == -1 ? sign(gayY) : 1) * ((resizingDirection.x == -1 && gayY > 2) ? -1 : 1),
                    abs(resizeStartSize.y + rotatedCursorDelta.y) * sign(wtf.y) * (resizingDirection.y == -1 ? sign(gayX) : 1) * ((resizingDirection.y == -1 && gayX > 2) ? -1 : 1)
                )
                pos.set(
                    resizeStartPos.added(size.divved(2, 2).sub(resizeStartSize.divved(2, 2)).mul(resizingDirection).rotate(-rotation))
                )
...
// Сделано: Масштабирование с Shift
